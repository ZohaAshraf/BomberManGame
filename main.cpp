...
const int row = 15;
const int col = 55;
const int numEnemies1 = 3;
const int numEnemies2 = 5;
char grid[row][col];
int countEnemies = 3;
int countEnemies = 5;
int player_life = 3;
...
int enemyPos[numEnemies1][2]; // 2D array for enemy positions
    ...
    while (true) {
        ...
        if (choice == '1') {
            ...
            while (true) {
                ...
                update_map(); 
                map(); 
                if (_kbhit()) { // Check for player input
                    input = _getch();
                    if (input == 27) { 
                        ...
                        else if (pauseChoice == 1)
                        {
                            ...
                            cout << "Tch..tch.tcch  press correct key" << endl;
                        }
                    }
                    else {
                        movePlayer(input);
                ...
                if (player_life == 0)
                {
                    ...
                    Sleep(3000);
                    char restartChoice;
                    cout << "                                                                                 Do you want to restart the game ? (Y or N)" << endl;
                    cin >> restartChoice;
        ...
        else if (choice == '4')
        {
            ...
            system("color 03");
            cout << "Tch.tch.tch....You blind press correct key!!!" << endl;// kiya bt h broo ... !!
            _getch(); 
        }
    }
    system("pause");
...
void bombdropsound()
{
    const wchar_t* soundFilePath = L"bombplant.wav";
...
void initializeEnemyPositions2() {
...
void powerup()
{
    ...
    {
        cout << "                                Power up !!! PLAYER LIFE INCREASED" << endl;
        powerupsound();
        player_life += 1;
    }
}
...
void loadingScreensound()
{
    const wchar_t* soundFilePath = L"loading_screen.wav";
...
void updateScores(int currentScore) {
    const int SIZE = 4;
    int highScores[SIZE] = { 0 };

    ifstream file("score2.txt");
    int count = 0;

    if (file.is_open()) {
        while (file >> highScores[count] && count < 3) {
            count++;
        }
        file.close();
    }
    ...
    if (outFile.is_open()) {
        for (int i = 0; i < 3; ++i) {
            if (highScores[i] != 0 || (highScores[i] == 0 && count > i)) { // Ensure we don't write leading zeros unless they're part of the scores read
...
void resetGame() {
    // Reset the map and player position
    initialize_map();
    playerX = row - 2;
    playerY = col / 2;
    ...
    // Reset enemies
    if (win == 0) {
        countEnemies = numEnemies1;
        initializeEnemyPositions(); // Initialize level 1 enemies
    }
    else if (win == 1) {
        countEnemies = numEnemies2;
        initializeEnemyPositions2(); // Initialize level 2 enemies
    }
...
void filehandler(int score) {
    const int TOTAL_SCORES = 3; // Maximum number of top scores to track
    int scores[TOTAL_SCORES + 1] = { 0, 0, 0, 0 }; // Array to store top 3 scores + new score
    int count = 0; // Number of scores read from the file

    // Read existing scores from the file
    ifstream inFile("score.txt");
    if (inFile.is_open()) {
        while (count < TOTAL_SCORES && inFile >> scores[count]) {
            count++;
        }
        inFile.close();
    }
    ...
    for (int i = 0; i < count - 1; i++) {
        for (int j = i + 1; j < count; j++) {
            if (scores[i] < scores[j]) {
                Swap(scores[i], scores[j]); // Use the custom Swap function
            }
        }
    }

    // Write back only the top 3 scores to the file
    ofstream outFile("score.txt");
    if (outFile.is_open()) {
        outFile << "LATEST SCORES ARE:" << endl;
        for (int i = 0; i < TOTAL_SCORES && i < count; i++) {
